...
7. To run energy minimization, make sure  ```tinker9nn.yaml``` exists in the folder. [results/optimization_results](results/optimization_results) contains the optimized structures from QM and AMOEBA+NN.
8. [results/md_results](results/md_results) contains the corresponding key file, and .arc trajectories of each species. Each .arc file contains the first and last frame from MD simulations. 
9. Details for running energy calculation, geometrty minimization and molecular dynamics simulation by calling Tinker-GPU could be found in [TinkerGPU_py](../../docs/tinker9nn_py.md) and [TinkerGPU_cuda](https://tinker9-manual.readthedocs.io/en/latest/m/key/nn.html). 
10. In the data.h5 file, each group has 6 datasets, ```amoeba.energy```, ```amoeba.force```,```atmoic_number```,```mp2.energy```,```mp2.force```,```coordinates```. If the group names contains 'mc', then it is from Monte-Carlo sampling; if it contains 'opt', then it is from geomerty optimization from high energy structures; otherwise samples are extracrted from MD simulations. ***a***n***b***o in the group name refers to ***a*** number of  ammonia and ***b*** number of water in the system. 
